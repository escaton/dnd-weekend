...
- **CD** — Docker image built in CI, pushed to fly registry, deployed to fly.io. Test env auto-deploys on push to main; prod deploys via manual `workflow_dispatch`. Two fly apps (test + prod) mapping to two Supabase projects (test + prod)
...
- **External services**: Supabase (auth, postgres, storage/S3) × 2 envs, fly.io × 2 apps, GitHub Container Registry (fly registry)
