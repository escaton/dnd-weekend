...
### 5. Migrations as pre-deploy gate in GitHub Actions

**Choice**: Both `deploy-test.yml` and `deploy-prod.yml` workflows run `drizzle-kit migrate` (Node.js + `postgres-js` over TCP to Supabase) before `wrangler pages deploy`. If migrations fail, the workflow exits non-zero and the deploy step never runs.

**Why**: Migrations need `postgres-js` TCP, not Hyperdrive. Running them in CI (Node.js) is the natural environment — the migration files and `drizzle-kit` stay as-is, only the execution context changes (Fly release_command → GitHub Action step).
...
- *Manual local migrations*: Developer runs `pnpm db:migrate` before deploy. Error-prone, no audit trail, no gate.
